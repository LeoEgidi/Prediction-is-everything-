...
Being the ultimate goal of science to find and reveal the truth, it is still ambiguous and controversial the role of prediction with respect to this task. Due to years characterized by larger and larger computational resources and vast amounts of data available, the same issue is worth to be investigated  in the modern statistical learning field too.  Is a method with a low predictive accuracy a bad method? Moreover, is a poor predictive accuracy able to falsify a posed theoretical model? Conversely, is a high predictive accuracy always constitutive of scientific success?  Statisticians argued a lot about the tension existing between prediction and explanation: in this paper, we wish to overcome this conflict by proposing a practical behavioral protocol to warn about possible uses and abuses of the predictive practices usually addressed by the modern statistical learning methods and to reason in terms of predictive inference, transparency, and falsification.
...
\noindent%
{\it Keywords:}  predictive accuracy; explanation; protocol; predictive inference; falsification.
\vfill
...
However, these two dimensions  appear often conflated, and any attempt to state the supremacy of one of the two entities over the other is in most cases misleading.  For such reason, we think the debate about the supposed prevalence of one of the two dimensions is local and application-dependent and, in general, wasteful.
Rather,  we could try to go beyond, by incorporating prediction in the explanation step, following some of the suggestions raised by the \emph{integrative modeling} approach \citep{hofman2021integrating}. As statisticians, we spent most of our efforts and our best years for reporting statistical estimates and standard errors, along with measures of \emph{statistical 
...
\emph{weak instrumentalism} (herafter, WI) approach, under which predictive accuracy is constitutive of scientific success only when the 
...
eventually reformulate their algorithms in order to improve the scientific inference.  

The remainder of this paper is organized as follows. Section \ref{sec:pred} reviews the role of prediction in science, whereas Section \ref{sec:role} depicts the role of prediction in statistical learning. Section \ref{sec:concl} concludes.
...
As statisticians required to build good models and fit complex data, we must warn statistical learning users about the role of prediction. Predictive accuracy is not always constitutive of scientific success, especially if we consider the field of application of the given method. \emph{Prediction is not everything}, however prediction could be seen as an alternative inferential tool to explain the models, starting from the observable rather than the parameters: as such, \emph{everything we observe is prediction}.  In this paper we propose a weak instrumentalist position for statistical learning, by highlighting and warning about the use and the abuse coming from the strong predictivism leading in the modern statistical learning. The result is a practical behavioral protocol organized in some main points such as: the evaluation of the fields of application (applicability); the assessment of the predictive task (prescription); the collection of those procedure connected with the data, as the choice of the training set and the test set to monitor a method (data treatment), and the corpus of tools required to build interpretable and transparent models  (interpretability).

Although not conclusive about the role of prediction for statistical practice, we feel this protocol could be useful for applied statisticians willing to mix the predictive accuracy and the interpretation of the methods.
