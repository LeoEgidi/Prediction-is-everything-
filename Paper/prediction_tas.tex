...
\color{blue}
Aggiungere esempio su chimica e Mendeleev?
...
- come si fa previsione nel moderno stat. learning., quali sono i criteri informativi, bias-variance trade-off, exp test mse, etc.

- inferenza e previsione sono solitamente viste come facce e oprazioni distinte, quasi antitetiche \citep{shmueli2010explain}, ma invece potremmo renderle molto piu vicine e complementari tramite concetto ampio di predictive inference \citep{billheimer2019predictive}.

- mettere in luce problemi predittivi e 'cattive pratiche' del moderno data-mining (shaking training set, tuning parameters, black-box, complexity), che spesso creano profonda distanza tra attori e 'modellisti'. 

- mettere in luce come una inferenza predittiva, che il paradigma Bayesiano già sposa abbondantemente, sia la strada fondamentale per una filosofia debolmente strumentalista, nonché il modo più trasparente per inquadrare le teorie scientifiche tramite modellazione statistica e superare i problemi descritti sopra: perche trasparenza? perche le cose osservate, a differenza dei parametri, sono sotto gli occhi di tutti e possono essere monitorate.

- quello della WI e' un vero e proprio protocollo comportamentale (rivedere Tab.1), con regole e norme

- citare explainable AI
***********************************
...
\color{blue}
arrivato qui: 26 gennaio
\color{black}


Another typical assessment  is given by the generalization performance of a learning method related to the predictive performance on a test set.
Unfortunately, it is commonly unfeasible to calculate the test mea-squared errors, because we are often in a situation in which we do not have any test data available.
% As suggested by \cite{hastie2009elements}, it does not seem possible to estimate the conditional error in \eqref{eq:test_MSE} appropriately, given only the information in the same training set $\tau$.  
For such reasons, we may introduce a related quantity, the \emph{expected test MSE}, or \emph{expected generalization error}, which is usually estimated by cross-validation \citep{hastie2009elements} and related statistical methods.
%\begin{equation}
...
Statistical procedures often incur in the \emph{bias-variance 
trade-off} \citep{james2013introduction}, the challenge is to find a compromise by controlling for both bias and variance.
The predictive goal is to select the model where the expected test MSE is lowest: the statisticians should try to minimize it by choosing the model that simultaneously has low variance and low bias.
...
%predictive robustness in decision trees, in the mid-1990s some data scientists argued that by aggregating many trees and perturbing the training set, using bagging , boosting 
...
aic : \cite{akaike1973information}

dic: \cite{spiegelhalter2002bayesian}

cart: \cite{breiman1984classification}

bagging: \cite{breiman1996bagging}

boosting: \cite{freund1996experiments}

rf: \citep{ho1995random}
...
\color{blue}
 Da rivedere 4.1 e modificare abbastanza, la 4.2 probabilmente molto da accorciare/cambiare.
\color{black}
