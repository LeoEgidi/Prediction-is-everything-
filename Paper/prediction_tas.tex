...
decide between good and bad statistical methods. Points (i)--(iv) in Section \ref{sec:good} refer to the \emph{strong instrumentalism} philosophical position, for which 
...
\color{red}
\begin{itemize}
\item[(1)] \textbf{Applicability} Social sciences are different from hard sciences. Then, statistical methods for social sciences should not be discarded just because of their poor predictive accuracy.
\item[(2)] \textbf{Prescription} predictive inference \citep{billheimer2019predictive}; Distribution of the observables, falsificationist Bayesianism \citep{gelman2017beyond};
\item[(3)] \textbf{Data-treatment} not invasive wrt predictive accuracy results; sensitivity analysis fully reported.
\item[(4)] \textbf{Interpretability, transparency, and falsification} joint model for parameters, observed and observable. Variability. Predictive distribution for NHST. Explanable AI
\end{itemize}
